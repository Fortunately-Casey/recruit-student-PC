...
export function TodateTime(chinadatetime) { //Fri Oct 31 18:00:00 UTC+0800 2008 
  if (chinadatetime) {
    var d = new Date(chinadatetime);
    var month = (d.getMonth() + 1) >= 10 ? (d.getMonth() + 1) : "0" + (d.getMonth() + 1);
    var date = d.getDate() >= 10 ? d.getDate() : "0" + d.getDate();
...
export function chinaDateTime(chinadatetime) {
  ...
  var T_pos = chinadatetime.indexOf('T');
  var Z_pos = chinadatetime.indexOf('Z');
  var year_month_day = chinadatetime.substr(0, T_pos);
  var hour_minute_second = chinadatetime.substr(T_pos + 1, Z_pos - T_pos - 1);
  var datetime = year_month_day + " " + hour_minute_second; // 2017-03-31 08:02:06
